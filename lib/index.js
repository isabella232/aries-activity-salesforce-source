...
// [ this.instanceUrl, "services/data", "v" + this.version ].join('/');
    ...
    async authenticateIfNeeded() {
        ...
        if (!this.serverUrl || !this.sessionId) {
            ...
            // now describe object
            const opts = {
                uri: `${this.instanceUrl}/services/data/v40.0/sobjects/${this.config.object}/describe`,
                method: 'GET',
                headers: {
            ...
            const describedObject = await rp(opts);
            const fields = describedObject.fields.map(x => x.name).join(',');
            console.log(fields);
            // now query for object records
            const timestampField = this.getTimestampField();
            const opts2 = {
                uri: `${this.instanceUrl}/services/data/v40.0/query`,
                method: 'GET',
                qs: {
                    q: `select ${fields} from ${this.config.object} where ${timestampField} >= ${moment(this.context.executionDate).format()} and  ${timestampField}< ${moment(this.context.nextExecutionDate).format()} order by ${timestampField} asc`,
                },
                headers: {
                    Authorization: `Bearer ${this.sessionId}`,
                },
            ...
            const { totalSize, done, records } = await rp(opts2);
            console.log(records);
...
class SalesforceStream extends Readable {
    constructor(context, config, opts) {
        super(opts);
        this.client = new SalesforceClient(context, config);
    }
