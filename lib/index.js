...
/**
 * @param name the field to escape
 * @param overrides a dict of custom overrides for the name
 * @return the escaped field name
 */
function escapeFieldName(name, overrides) {
...
function mapper(config, describedObject, fieldNameOverrides) {
    return function (obj) {
        const dateFields = describedObject.fields.filter(x => x.type === 'date' || x.type === 'datetime').map(x => x.name);
        timestampFields(obj, dateFields);
        // MUST snakecase keys for redshift
        return mapKeys(obj, (value, key) => escapeFieldName(key, fieldNameOverrides));
    };
}
...
function defaultForType(avroType) {
    if (avroType === 'boolean') return false;
    else if (avroType === 'string') return '';
    else if (avroType === 'int' || avroType === 'long') return 0;
    else if (avroType === 'double') return 0.0;
    return null;
}

function avroField(field, fieldNameOverrides) {
    const { name, type, nillable } = field;
    const avroType = mapping[type];
    const fieldObj = {
        name: escapeFieldName(name, fieldNameOverrides),
    };
    if (type === 'date' || type === 'datetime') {
        if (nillable) {
            fieldObj.type = ['null', { type: 'long', logicalType: 'timestamp-millis' }];
            fieldObj.default = null;
        } else {
            fieldObj.type = { type: 'long', logicalType: 'timestamp-millis' };
            fieldObj.default = 0;
        }
    } else if (nillable) {
        fieldObj.type = ['null', avroType];
        fieldObj.default = null;
    } else {
        fieldObj.type = avroType;
        fieldObj.default = defaultForType(avroType);
    }

    return fieldObj;
}

function avroSchemaFromDescribedObject(obj, fieldNameOverrides = {}) {
    const fields = obj.fields.filter(x => mapping.hasOwnProperty(x.type)).map(x => avroField(x, fieldNameOverrides));
...
async function source(context, config) {
    ...
    const avroSchema = avroSchemaFromDescribedObject(describedObject, config.fieldNameOverrides);
    const encoder = new avro.streams.BlockEncoder(avroSchema, { batchSize: 1, codec: 'deflate' });
    return stream.flatten().map(mapper(config, describedObject, config.fieldNameOverrides)).pipe(encoder);
}
